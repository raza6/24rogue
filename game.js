...
function init() {
	...
	availableCells = availableCells.filter((cell, idx, arr) =>
		idx === arr.findIndex((c) => (
	...
	// Create items
	items = new Array(getRandom(1, 4, rngGen) + currentLvl);
...
function playerAct(x, y) {
	...
	switch (map[x][y]) {
		...
		case CELL.ENEMY_2:
			...
			let strengthBoost = player.strength ? 2 : 1;
